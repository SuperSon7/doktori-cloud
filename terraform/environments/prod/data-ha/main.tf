...
# Nodes are spread across 3 AZs (AZ-a, AZ-c, AZ-b) for fault tolerance.
#
# DNS records (auto-registered by User Data):
#   data-1.prod.doktori.internal  (AZ-a)
#   data-2.prod.doktori.internal  (AZ-c)
#   data-3.prod.doktori.internal  (AZ-b)
#
# Prerequisites:
#   1. prod/base must be applied (VPC, subnets, Route53 zone)
#   2. SSM parameters must exist:
#      /doktori/prod/SPRING_REDIS_PASSWORD           (SecureString)
#      /doktori/prod/SPRING_RABBITMQ_USERNAME         (SecureString)
#      /doktori/prod/SPRING_RABBITMQ_PASSWORD         (SecureString)
#      /doktori/prod/RABBITMQ_ERLANG_COOKIE           (SecureString)
# =============================================================================
...
# Existing subnets: private_db (AZ-a, 10.1.32.0/24), private_rds (AZ-c, 10.1.40.0/24)
# New subnet: private_data_b (AZ-b) for better AZ spread
# Using 10.1.36.0/24 — between private_db and private_rds ranges
# -----------------------------------------------------------------------------

...
분산 러너 (3x EC2, 별도 AWS 계정)
    │
    │ HTTPS (api.doktori.kr → ALB 직접, CloudFront 우회)
    ▼
┌─────────────────────────────────┐
│  Public ALB (api.doktori.kr)    │  TLS 1.2/1.3 termination
├─────────────────────────────────┤
│  /api/*  → K8s NodePort 30080   │
│  /ws/*   → K8s NodePort 30080   │
└───────────┬─────────────────────┘
            ▼
┌─────────────────────────────────┐
│  NGF (NGINX Gateway Fabric)     │
├─────────────────────────────────┤
│  /api/*        → api-svc:8080   │  API Pod × 2
│  /api/chat-*   → chat-svc:8081  │  Chat Pod × 2
│  /ws/chat      → chat-svc:8081  │  WebSocket (STOMP, 1h timeout)
└─────────────────────────────────┘
         │              │
...
```bash
export BASE_URL="https://api.doktori.kr/api"    # 프로덕션 API (ALB 직접, CF 우회)
